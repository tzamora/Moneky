...
public class MiniMonkeyMediator : Unit
{
	public int id = 0;
	...
	public int movementFramePointer = 0;
	
	private bool minimonkeyActivated = false;
	
	public bool DetachMonkey = false;
	
	private UnitPlayer Player;
	
	public MonkeyTypeEnum MonkeyType;
	
	public GameObject MonkeyTypePrefab;
	
	public void EnableMiniMonkey(UnitPlayer parentPlayer)
	{
		if(!minimonkeyActivated)
		{
			//
			// procede to make the monkey follow me
			//
	...
	{	
		//
		// start following the animation from the current animation
		//
		
		movementFramePointer = InputControllerMediator.movementInFrames.Count - (5 * id);
		
		//
		// wait a frame to get the next player position
		//
		while(true)
		{
			transform.position = InputControllerMediator.movementInFrames[movementFramePointer];
			
			if(Player.side > 0)
			{
				this.gameObject.GetComponent<tk2dSprite>().FlipX = false;
			...
			if(Player.side < 0)
			{
				this.gameObject.GetComponent<tk2dSprite>().FlipX = true;
